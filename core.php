<?php
		...
		if( ! $options ) {
			...
			if( ! $db_option ) {
				...
				if(!empty($_POST) && isset($_POST['wpe_sharing'])){
					//pree($_POST);exit;
					update_option( 'wpe_sharing', $_POST['wpe_sharing'] );
					$db_option = get_option( 'wpe_sharing', array());
				}
			}
	...
	function wpe_plugin_links($links) { 
		...
		$settings_link = '<a href="options-general.php?page=wp-easy-sharing">Settings</a>';
		...
		if($wpe_pro){
			...
			$wpe_premium_link = '<a href="'.$wpe_premium_link.'" title="Go Premium" target=_blank>Go Premium</a>'; 
